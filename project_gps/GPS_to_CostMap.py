...
def read_all_GPS_data():
    ...
    for gps_file in list_of_files:
        output_file_path = '/path_kml_files/' + gps_file.split('\\')[1].split('.')[0] + '.kml'
        output_file_left_right = '/left_right_kml_files/' + gps_file.split('\\')[1].split('.')[0] + '.kml'
        output_file_stop = '/stop_kml_files/' + gps_file.split('\\')[1].split('.')[0] + '.kml'
        ...
        write_to_left_right_kml(output_file_left_right, left_turns, right_turns)
...
def read_input_data(gps_file, kml_directory):
    ...
    # writes the kml data on to the corresponding files
    write_to_path_kml(kml_directory + '/kml_path.kml', processed_data_df.values.tolist())
    write_to_left_right_kml(kml_directory + '/kml_left_right.kml', left_turns, right_turns)
...
def main():
    if len(sys.argv) != 3:
        read_all_GPS_data()
        print("Usage: python3 GPS_to_KML.py GPS_FileName.txt KML_Directory")
    else:
